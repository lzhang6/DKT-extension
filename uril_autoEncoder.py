...
class AutoEncoder(object):
    ...
    def save_weights(self, path):
        ...
        """
        for i in range(len(self.encoding_matrices)):
            weightpath = path+'/weights_'+str(i+1)+'.csv'
            wt = self.encoding_matrices[i].eval(self.sess)
            np.savetxt(weightpath,wt)

            baispath = path+'/bias_'+str(i+1)+'.csv'
            bs = self.encoding_biases[i].eval(self.sess)
            np.savetxt(baispath,bs)
        print ("save the weight to the direct:\t",path)
